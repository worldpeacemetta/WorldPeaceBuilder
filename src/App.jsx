// MacroTracker application — custom build with extended dashboard visuals,
// editable food database tooling, and profile-aware goal scheduling.
...
function sanitizeProfileHistory(value) {
  ...
  value.forEach((item) => {
    ...
    const iso = typeof item.date === "string" && ISO_DATE_RE.test(item.date) ? item.date : null;
    if (!iso) return;

    const weight = toNumber(item.weightKg, Number.NaN);
    const bodyFat = toNumber(item.bodyFatPct, Number.NaN);
    ...
    entries.set(iso, {
      date: iso,
      ...(hasWeight ? { weightKg: +weight.toFixed(1) } : {}),
      ...(hasBodyFat ? { bodyFatPct: +bodyFat.toFixed(1) } : {}),
    });
  });
...
function ensureSettings(value) {
  ...
  return {
    ...
    profile,
    profileHistory: sanitizeProfileHistory(base.profileHistory),
  };
}
...
export default function MacroTrackerApp(){
  ...
  useEffect(() => {
    ...
    const apply = () => {
      const dark = theme === "system" ? mql.matches : theme === "dark";
  ...
  const macrosByDate = useMemo(() => {
    const map = new Map();
    entries.forEach((entry) => {
      if (!ISO_DATE_RE.test(entry.date ?? "")) return;
      const food = foods.find((item) => item.id === entry.foodId);
      if (!food) return;
      const macros = scaleMacros(food, entry.qty);
      const existing = map.get(entry.date) || { kcal: 0, fat: 0, carbs: 0, protein: 0 };
  ...
  const totalsForDate = useCallback((iso) => {
    if (!ISO_DATE_RE.test(iso ?? "")) {
      return { kcal: 0, fat: 0, carbs: 0, protein: 0 };
    }
    const totals = macrosByDate.get(iso);
    if (!totals) {
      return { kcal: 0, fat: 0, carbs: 0, protein: 0 };
    }
    return {
  ...
  const stickyTotals = useMemo(() => totalsForDate(stickyDate), [totalsForDate, stickyDate]);
  const totalsForCard = useMemo(() => totalsForDate(logDate), [totalsForDate, logDate]);
  ...
  const goalTotals = useMemo(() => totalsForDate(goalDate), [totalsForDate, goalDate]);
  ...
  }, [goalDate, goalValuesForDate, totalsForDate]);

  const weightHistory = useMemo(() => sanitizeProfileHistory(settings.profileHistory), [settings.profileHistory]);
  ...
  const weightTrend = useMemo(() => {
    ...
    const weightedEntries = weightHistory.filter((entry) => Number.isFinite(entry.weightKg));
    const data = weightedEntries
    ...
    const latestWeightEntry = [...weightedEntries].reverse().find((entry) => Number.isFinite(entry.weightKg));

    return {
      data,
      latestWeight: latestWeightEntry?.weightKg ?? null,
      latestDate: latestWeightEntry?.date ?? null,
    };
  }, [weightHistory]);

  const foodLogging = useMemo(() => {
    const today = startOfDay(new Date());
    const start = subDays(today, 29);
    const days = eachDayOfInterval({ start, end: today });
    const cells = days.map((date) => {
      const iso = toISODate(date);
      const logs = entriesByDate.get(iso) ?? [];
      return {
        iso,
    ...
    const loggedDays = cells.filter((cell) => cell.hasLogs).length;
    const thisWeekCells = cells.slice(-7);
    const thisWeekCount = thisWeekCells.filter((cell) => cell.hasLogs).length;
  ...
  const handleSaveBodyProfile = useCallback(() => {
    const iso = todayISO();
    setSettings((prev) => {
      const weight = toNumber(prev?.profile?.weightKg, Number.NaN);
      const bodyFat = toNumber(prev?.profile?.bodyFatPct, Number.NaN);
      const hasWeight = Number.isFinite(weight) && weight > 0;
      const hasBodyFat = Number.isFinite(bodyFat) && bodyFat > 0;
      ...
      const history = sanitizeProfileHistory(prev?.profileHistory);
      const existing = history.find((entry) => entry.date === iso);

      if (
        existing &&
        existing.weightKg === nextEntry.weightKg &&
        existing.bodyFatPct === nextEntry.bodyFatPct
      ) {
        return prev;
      }

      const updated = [...history.filter((entry) => entry.date !== iso), nextEntry].sort((a, b) =>
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        <Tabs value={tab} onValueChange={setTab}>
          ...
          <TabsContent value="dashboard" className="mt-6 space-y-6">
            ...
            <div className="grid md:grid-cols-4 gap-4">
              <AvgTile label="Avg (7d)" totalsByDate={macrosByDate} days={7} />
              <AvgTile label="Avg (MTD)" totalsByDate={macrosByDate} from={startOfMonth(new Date())} />
              <AvgTile label="Avg (QTD)" totalsByDate={macrosByDate} from={startOfQuarter(new Date())} />
              <AvgTile label="Avg (YTD)" totalsByDate={macrosByDate} from={startOfYear(new Date())} />
            </div>

            <div className="grid md:grid-cols-2 gap-4">
              <WeightTrendCard trend={weightTrend} />
              <FoodLoggingCard summary={foodLogging} />
            </div>
          </TabsContent>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="col-span-2 flex justify-end">
                        <Button onClick={handleSaveBodyProfile}>Save</Button>
                      </div>
                    </div>
                  </div>
...
function AvgTile({ label, totalsByDate, days, from }) {
  const todayIso = toISODate(startOfDay(new Date()));

  const dateKeys = useMemo(() => {
    if (!totalsByDate || typeof totalsByDate.keys !== "function") {
      return [];
    }
    return Array.from(totalsByDate.keys());
  }, [totalsByDate]);

  const selectedDates = useMemo(() => {
    if (!dateKeys.length) return [];
    if (days) {
      return [...dateKeys].sort((a, b) => b.localeCompare(a)).slice(0, days);
    }
    if (from) {
      const fromIso = toISODate(startOfDay(from));
      return dateKeys.filter((d) => d >= fromIso && d <= todayIso).sort();
    }
    return [...dateKeys];
  }, [dateKeys, days, from, todayIso]);

  const count = Math.max(1, selectedDates.length);
  const total = selectedDates.reduce(
    (acc, date) => {
      const totals = totalsByDate?.get?.(date) || { kcal: 0, fat: 0, carbs: 0, protein: 0 };
      return {
        kcal: acc.kcal + (totals.kcal ?? 0),
        fat: acc.fat + (totals.fat ?? 0),
        carbs: acc.carbs + (totals.carbs ?? 0),
  ...
  const avg = selectedDates.length === 0
    ? { kcal: 0, fat: 0, carbs: 0, protein: 0 }
    : {
        kcal: +(total.kcal / count).toFixed(0),
  ...
  const bars = [
    { key: "kcal", label: "Calories", unit: "kcal", value: avg.kcal, theme: MACRO_THEME.kcal },
    { key: "protein", label: "Protein", unit: "g", value: avg.protein, theme: MACRO_THEME.protein },
    { key: "carbs", label: "Carbs", unit: "g", value: avg.carbs, theme: MACRO_THEME.carbs },
    { key: "fat", label: "Fat", unit: "g", value: avg.fat, theme: MACRO_THEME.fat },
  ];

  const maxValue = bars.reduce((max, item) => Math.max(max, item.value ?? 0), 0) || 1;
  ...
  return (
    <Card className="flex flex-col">
      ...
      <CardContent className="flex-1 space-y-3 pt-0">
        {bars.map((bar) => {
          const rawPercent = maxValue > 0 ? (bar.value / maxValue) * 100 : 0;
          const width = Math.min(rawPercent > 0 ? Math.max(rawPercent, 6) : 0, 100);
          return (
            <div key={bar.key} className="space-y-1">
              <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                <span className="flex items-center gap-2">
                  <span
                    className="inline-flex h-2 w-2 rounded-full"
                    style={{ backgroundImage: `linear-gradient(135deg, ${bar.theme.gradientFrom}, ${bar.theme.gradientTo})` }}
                    aria-hidden="true"
                  />
                  {bar.label}
                </span>
                <span className="font-semibold text-slate-700 dark:text-slate-100">
                  ...
                  <span className="ml-1 text-[10px] font-normal uppercase text-slate-400 dark:text-slate-500">{bar.unit}</span>
                </span>
              </div>
              <div className="h-2.5 w-full rounded-full bg-slate-200/70 dark:bg-slate-800/60">
                <div
                  className="h-full rounded-full shadow-sm"
                  style={{
                    width: `${width}%`,
                    backgroundImage: `linear-gradient(90deg, ${bar.theme.gradientFrom}, ${bar.theme.gradientTo})`,
                  }}
                />
              </div>
            </div>
...
function WeightTrendCard({ trend }) {
  const data = trend?.data ?? [];
  const latestWeight = trend?.latestWeight ?? null;
  const latestDate = trend?.latestDate ?? null;
      ...
      <CardContent className="flex-1 space-y-3 pt-1">
        {!data.length ? (
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Save a weight update in your profile to unlock this trend.
          </p>
        ) : (
          <div className="h-44">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data} margin={{ top: 10, right: 8, bottom: 0, left: 0 }}>
                ...
                <RTooltip
                  cursor={{ stroke: COLORS.violet, strokeOpacity: 0.25 }}
        ...
        <div>
          ...
          <div className="text-2xl font-semibold text-slate-900 dark:text-slate-100">
            {latestWeight ? `${formatNumber(latestWeight)} kg` : "--"}
          </div>
          {latestDate && (
            <div className="text-xs text-slate-500 dark:text-slate-400">Saved {format(new Date(latestDate), "PP")}</div>
          )}
        </div>
      </CardContent>
    </Card>
...
function FoodLoggingCard({ summary }) {
  ...
  const thisWeekCount = summary?.thisWeekCount ?? 0;
  ...
  return (
    <Card className="flex flex-col">
      ...
      <CardContent className="flex-1 space-y-4 pt-1">
        {!cells.length ? (
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Log your meals to see streaks and habits build up here.
          </p>
        ) : (
          <div className="space-y-2">
            <div className="grid grid-cols-7 gap-1.5">
              {cells.map((cell) => (
                <div
                  key={cell.iso}
                  className={cn(
                    "h-6 rounded-lg border border-transparent transition-all",
                    cell.hasLogs
                      ? "shadow-sm"
                      : "bg-slate-200/60 dark:bg-slate-800/60",
                    cell.isToday &&
                      "ring-2 ring-offset-1 ring-offset-white ring-cyan-300 dark:ring-cyan-400 dark:ring-offset-slate-950"
                  )}
                  style={
                    cell.hasLogs
                      ? {
                          backgroundImage: `linear-gradient(135deg, ${MACRO_THEME.carbs.gradientFrom}, ${MACRO_THEME.carbs.gradientTo})`,
                        }
                      : undefined
                  }
                  title={`${cell.label}${cell.count ? ` · ${cell.count} log${cell.count > 1 ? "s" : ""}` : ""}`}
                  aria-label={`${cell.label}: ${cell.hasLogs ? `${cell.count} log${cell.count === 1 ? "" : "s"}` : "No logs"}`}
                />
              ))}
            </div>
            <div className="flex items-center gap-3 text-[11px] text-slate-500 dark:text-slate-400">
              <div className="flex items-center gap-2">
                <div
                  className="h-3 w-3 rounded-md border border-transparent"
                  style={{
                    backgroundImage: `linear-gradient(135deg, ${MACRO_THEME.carbs.gradientFrom}, ${MACRO_THEME.carbs.gradientTo})`,
                  }}
                />
              ...
              <div className="flex items-center gap-2">
                <div className="h-3 w-3 rounded-md bg-slate-200/60 dark:bg-slate-800/60" />
                No log
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
          <span className="font-medium text-slate-700 dark:text-slate-200">
            {loggedDays}/{totalDays} days logged
          </span>
          <span>{thisWeekCount}/7 this week</span>
        </div>
      </CardContent>
    </Card>
  );
